...
export async function Deploy(core: Core, dir: string, token: string, id?: string): Promise<{ reply: string, success: boolean }> {
	...
	let deploy: boolean = await exe.run(`netlify deploy --dir=${dir} --prod`, {env: env, silent: false});
